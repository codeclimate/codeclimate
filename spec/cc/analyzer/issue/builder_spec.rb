  ...
  describe Issue::Builder do
    ...
    describe "#run" do
      it "constructs an appropriate error for invalid json" do
        builder = Issue::Builder.new("foo", "engine-name").tap(&:run)
      ...
      it "constructs an appropriate error for issue missing location" do
        builder = Issue::Builder.new(sample_issue_json("location" => {}), "engine-name").tap(&:run)
      ...
      it "constructs an appropriate error for invalide issue" do
        builder = Issue::Builder.new(sample_issue_json("check_name" => nil), "engine-name").tap(&:run)
      ...
      it "constructs the adapted issue document" do
        builder = Issue::Builder.new(sample_issue_json, "engine-name").tap(&:run)
